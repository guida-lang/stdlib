...
for viewing Guida data structures.

**Note:** This is not available with `guida make --optimize` which gets rid of
...
**Note:** This is not available with `guida make --optimize` because (1) it
...
terminal. We may have a package in Guida for this someday, but browser
applications are the primary focus of platform development for now.

-}
log : String -> a -> a
log =
    Guida.Kernel.Debug.log
...
The Guida compiler recognizes each `Debug.todo` so if you run into it, you get
an **uncatchable runtime exception** that includes the module name and line
number.

**Note:** This is not available with `guida make --optimize` or packages. The
...
**Note:** For the equivalent of try/catch error handling in Guida, use modules
