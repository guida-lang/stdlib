module Test.Html.Internal.GuidaHtml.InternalTypes exposing
    ( GuidaHtml(..), TextTagRecord, NodeRecord, CustomNodeRecord, MarkdownNodeRecord
    , Facts, Tagger, EventHandler, ElementKind(..)
    , Attribute(..), AttributeRecord, NamespacedAttributeRecord, PropertyRecord, EventRecord
    , decodeGuidaHtml, emptyFacts, toElementKind, decodeAttribute
    )

{-| Internal types used to represent Guida Html in pure Guida

@docs GuidaHtml, TextTagRecord, NodeRecord, CustomNodeRecord, MarkdownNodeRecord
...
@docs decodeGuidaHtml, emptyFacts, toElementKind, decodeAttribute

-}

import Dict exposing (Dict)
import Json.Decode exposing (field)
import Test.Html.Internal.GuidaHtml.Constants as Constants exposing (..)
import Test.Html.Internal.GuidaHtml.Helpers exposing (..)
import Test.Html.Internal.GuidaHtml.Markdown exposing (..)
import Test.Internal.KernelConstants exposing (kernelConstants)
import VirtualDom


{-| Type tree for representing Guida's Html
...
{-| Type for representing Guida's Attributes
...
{-| decode a json object into GuidaHtml, you have to pass a function that decodes
events from Html Nodes. If you don't want to decode event msgs, you can ignore it:

    decodeGuidaHtml (\_ _ -> VirtualDom.Normal (Json.Decode.succeed ())) jsonHtml
...
-}
decodeGuidaHtml : (List Tagger -> EventHandler -> VirtualDom.Handler msg) -> Json.Decode.Decoder (GuidaHtml msg)
decodeGuidaHtml eventDecoder =
    contextDecodeGuidaHtml (HtmlContext [] eventDecoder)


contextDecodeGuidaHtml : HtmlContext msg -> Json.Decode.Decoder (GuidaHtml msg)
contextDecodeGuidaHtml context =
        ...
        |> Json.Decode.andThen
            (\nodeType ->
                if nodeType == kernelConstants.virtualDom.nodeTypeText then
                    ...
                    field kernelConstants.virtualDom.node (contextDecodeGuidaHtml context)
...
{-| decode a tagger
-}
decodeTagger : HtmlContext msg -> Json.Decode.Decoder (GuidaHtml msg)
    ...
    Json.Decode.field kernelConstants.virtualDom.tagger Json.Decode.value
        |> Json.Decode.andThen
            (\tagger ->
                let
                    nodeDecoder =
                        contextDecodeGuidaHtml (HtmlContext (taggers ++ [ tagger ]) eventDecoder)
                in
...
decodeKeyedNode context =
    let
        -- guida stores keyed nodes as tuples
        -- we only want to decode the html, in the second property
        decodeSecondNode =
            Json.Decode.field "b" (contextDecodeGuidaHtml context)
    in
...
decodeNode context =
    Json.Decode.map4 NodeRecord
        ...
        (field kernelConstants.virtualDom.kids (Json.Decode.list (contextDecodeGuidaHtml context)))
...
-}
decodeCustomNode : HtmlContext msg -> Json.Decode.Decoder (GuidaHtml msg)
decodeCustomNode context =
    Json.Decode.oneOf
