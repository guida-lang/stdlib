...
node tag =
    Guida.Kernel.VirtualDom.node (Guida.Kernel.VirtualDom.noScript tag)
...
nodeNS namespace tag =
    Guida.Kernel.VirtualDom.nodeNS namespace (Guida.Kernel.VirtualDom.noScript tag)
...
1.  Attributes &mdash; You can set things in HTML itself. So the `class`
    in `<div class="greeting"></div>` is called an _attribute_.

2.  Properties &mdash; You can also set things in JS. So the `className`
    in `div.className = 'greeting'` is called a _property_.

So the `class` attribute corresponds to the `className` property. At first
glance, perhaps this distinction is defensible, but it gets much crazier.
_There is not always a one-to-one mapping between attributes and properties!_
...
Notice that you must give the _property_ name, so we use `htmlFor` as it
would be in JavaScript, not `for` as it would appear in HTML.

-}
property : String -> Json.Value -> Attribute msg
property key value =
    Guida.Kernel.VirtualDom.property
        (Guida.Kernel.VirtualDom.noInnerHtmlOrFormAction key)
        (Guida.Kernel.VirtualDom.noJavaScriptOrHtmlJson value)
...
Notice that you must give the _attribute_ name, so we use `for` as it would
be in HTML, not `htmlFor` as it would appear in JS.

-}
attribute : String -> String -> Attribute msg
attribute key value =
    Guida.Kernel.VirtualDom.attribute
        (Guida.Kernel.VirtualDom.noOnOrFormAction key)
        (Guida.Kernel.VirtualDom.noJavaScriptOrHtmlUri value)
...
attributeNS namespace key value =
    Guida.Kernel.VirtualDom.attributeNS
        namespace
        (Guida.Kernel.VirtualDom.noOnOrFormAction key)
        (Guida.Kernel.VirtualDom.noJavaScriptOrHtmlUri value)
...
mapAttribute =
    Guida.Kernel.VirtualDom.mapAttribute
...
keyedNode tag =
    Guida.Kernel.VirtualDom.keyedNode (Guida.Kernel.VirtualDom.noScript tag)
...
keyedNodeNS namespace tag =
    Guida.Kernel.VirtualDom.keyedNodeNS namespace (Guida.Kernel.VirtualDom.noScript tag)
