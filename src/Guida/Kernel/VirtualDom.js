...
function _VirtualDom_render(vNode, eventNode) {
	...
	if (tag === __2_TAGGER) {
		...
		domNode.guida_event_node_ref = subEventRoot;
		return domNode;
	}
...
function _VirtualDom_applyEvents(domNode, eventNode, events) {
	var allCallbacks = domNode.guidaFs || (domNode.guidaFs = {});
...
var _VirtualDom_POSTFIX = '_guidaW6BL';
...
function _VirtualDom_addDomNodesHelp(domNode, vNode, patches, i, low, high, eventNode) {
	...
	if (tag === __2_TAGGER) {
		...
		return _VirtualDom_addDomNodesHelp(domNode, subNode, patches, i, low + 1, high, domNode.guida_event_node_ref);
	}
...
function _VirtualDom_applyPatch(domNode, patch) {
	switch (patch.$) {
		...
		case __3_TAGGER:
			if (domNode.guida_event_node_ref) {
				domNode.guida_event_node_ref.__tagger = patch.__data;
			}
			else {
				domNode.guida_event_node_ref = { __tagger: patch.__data, __parent: patch.__eventNode };
			}
			return domNode;
...
function _VirtualDom_applyPatchRedraw(domNode, vNode, eventNode) {
	...
	if (!newNode.guida_event_node_ref) {
		newNode.guida_event_node_ref = domNode.guida_event_node_ref;
	}
