...
function _Debugger_scroll(popout) {
	return __Scheduler_binding(function (callback) {
		if (popout.__doc) {
			var msgs = popout.__doc.getElementById('guida-debugger-sidebar');
...
function _Debugger_messageToString(value) {
	...
	if (['Array_guida_builtin', 'Set_guida_builtin', 'RBNode_guida_builtin', 'RBEmpty_guida_builtin'].indexOf(value.$) >= 0) {
		return '…';
	}
...
function _Debugger_blocker(event) {
	...
	for (var node = event.target; node; node = node.parentNode) {
		if (isScroll ? node.id === 'guida-debugger-details' : node.id === 'guida-debugger-overlay') {
			return;
		}
	}
