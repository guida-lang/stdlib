...
import Guida.Kernel.Json exposing (decodePrim, expecting)
...
function _File_uploadOneOrMore(mimes) {
	return __Scheduler_binding(function (callback) {
		...
		_File_node.accept = A2(__String_join, ',', mimes);
		_File_node.addEventListener('change', function (event) {
			var guidaFiles = __List_fromArray(event.target.files);
			callback(__Scheduler_succeed(__Utils_Tuple2(guidaFiles.a, guidaFiles.b)));
		});
		_File_click(_File_node);
	});
}
