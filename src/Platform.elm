...
Fundamentally, Guida needs maybe 10 of them total. I get that people are smart,
...
{-| A `Program` describes a Guida program! How does it react to input? Does it
...
This is great if you want to use Guida as the &ldquo;brain&rdquo; for something
else. For example, you could send messages out ports to modify the DOM, but do
all the complex logic in Guida.
...
```javascript
var app = Guida.MyThing.init();
```

If you _do_ want to control the user interface in Guida, the [`Browser`][browser]
module has a few ways to create that kind of `Program` instead!

[headless]: https://en.wikipedia.org/wiki/Headless_software
[browser]: /packages/guida-lang/stdlib/latest/Browser
...
sendToApp =
    Guida.Kernel.Platform.sendToApp
...
sendToSelf =
    Guida.Kernel.Platform.sendToSelf
