...
can improve error messages a ton! This is how the Guida compiler produces
...
The "context" technique is how the Guida compiler can say "I think I am parsing a
...
**Notice that `boolean` uses `boolean` in its definition!** In Guida, you can
...
If you want to disable literals like `.123` (like in Guida) you could write
something like this:

    guidaFloat : Parser Float
    guidaFloat =
        oneOf
            [ symbol "."
...
In fact, `multiComment "/*" "*/" NotNestable` _is_ implemented like that! It is
