...
import Bitwise
import Guida.JsArray as JsArray exposing (JsArray)
...
type Array a
    = {-
         ...
         * tail : JsArray a = The tail of the array. Inserted into tree when
         ...
         fast.
      -}
      Array_guida_builtin Int Int (Tree a) (JsArray a)
...
empty =
    ...
    -}
    Array_guida_builtin 0 shiftStep JsArray.empty JsArray.empty
...
isEmpty (Array_guida_builtin len _ _ _) =
    len == 0
...
-}
length : Array a -> Int
length (Array_guida_builtin len _ _ _) =
    len
...
get index (Array_guida_builtin len startShift tree tail) =
...
set index value ((Array_guida_builtin len startShift tree tail) as array) =
    if index < 0 || index >= len then
        array

    else if index >= tailIndex len then
        Array_guida_builtin len startShift tree <|
...
push a ((Array_guida_builtin _ _ _ tail) as array) =
...
unsafeReplaceTail newTail (Array_guida_builtin len startShift tree tail) =
    let
        originalTailLen =
            JsArray.length tail
...
toIndexedList ((Array_guida_builtin len _ _ _) as array) =
    let
...
foldr func baseCase (Array_guida_builtin _ _ tree tail) =
    let
        helper node acc =
            case node of
                SubTree subTree ->
...
foldl func baseCase (Array_guida_builtin _ _ tree tail) =
    let
        helper node acc =
            case node of
                SubTree subTree ->
...
map func (Array_guida_builtin len startShift tree tail) =
    let
        helper node =
            case node of
                SubTree subTree ->
...
indexedMap func (Array_guida_builtin len _ tree tail) =
    let
        helper node builder =
            case node of
                SubTree subTree ->
...
append ((Array_guida_builtin _ _ _ aTail) as a) (Array_guida_builtin bLen _ bTree bTail) =
...
appendHelpTree toAppend ((Array_guida_builtin len _ tree tail) as array) =
    let
        appended =
...
translateIndex index (Array_guida_builtin len _ _ _) =
    let
        posIndex =
            if index < 0 then
                len + index
...
sliceRight end ((Array_guida_builtin len startShift tree tail) as array) =
    if end == len then
        array

    else if end >= tailIndex len then
        Array_guida_builtin end startShift tree <|
...
sliceLeft from ((Array_guida_builtin len _ tree tail) as array) =
    if from == 0 then
        array

    else if from >= tailIndex len then
        Array_guida_builtin (len - from) shiftStep JsArray.empty <|
...
builderFromArray (Array_guida_builtin len _ tree tail) =
    let
        helper node acc =
            case node of
                SubTree subTree ->
