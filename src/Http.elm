...
jsonBody value =
  Guida.Kernel.Http.pair "application/json" (Encode.encode 0 value)
...
multipartBody parts =
  Guida.Kernel.Http.pair "" (Guida.Kernel.Http.toFormData parts)
...
bytesPart key mime bytes =
  Guida.Kernel.Http.pair key (Guida.Kernel.Http.bytesToBlob mime bytes)
...
expectStringResponse toMsg toResult =
  Guida.Kernel.Http.expect "" identity (toResult >> toMsg)
...
expectBytesResponse toMsg toResult =
  Guida.Kernel.Http.expect "arraybuffer" Guida.Kernel.Http.toDataView (toResult >> toMsg)
...
task
  ...
  -> Task x a
task r =
  Guida.Kernel.Http.toTask () resultToTask
    { method = r.method
    , headers = r.headers
    , url = r.url
    , body = r.body
...
stringResolver =
  Guida.Kernel.Http.expect "" identity
...
bytesResolver =
  Guida.Kernel.Http.expect "arraybuffer" Guida.Kernel.Http.toDataView
  ...
  case cmd of
    ...
    Request r ->
      Request
        ...
        , url = r.url
        , body = r.body
        , expect = Guida.Kernel.Http.mapExpect func r.expect
        , timeout = r.timeout
        , tracker = r.tracker
  ...
  case cmds of
    ...
    cmd :: otherCmds ->
      case cmd of
        ...
        Request req ->
          Process.spawn (Guida.Kernel.Http.toTask router (Platform.sendToApp router) req)
            |> Task.andThen (\pid ->
                  case req.tracker of
                    Nothing ->
                      updateReqs router otherCmds reqs
