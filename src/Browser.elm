...
{-| This module helps you set up a Guida `Program` with functions like
...
{-| Create an HTML element managed by Guida. The resulting elements are easy to
embed in larger JavaScript projects. Try it out on something small. If it works,
great, do more! If not, revert, no big deal.

Unlike a [`sandbox`](#sandbox), an `element` can talk to the outside world in
a couple ways:

  - `Cmd` &mdash; you can “command” the Guida runtime to do stuff, like HTTP.
  - `Sub` &mdash; you can “subscribe” to event sources, like clock ticks.
  - `flags` &mdash; JavaScript can pass in data when starting the Guida program
...
{-| Create an HTML document managed by Guida. This expands upon what `element`
...
>
> 2.  Compile your Guida code to JavaScript with `guida make --output=guida.js` and
>     then make your own HTML file that loads `guida.js` and the CSS file you want.
...
> the responsibility of the `view` function in Guida. So we do it differently!
...
application :
    ...
    -> Program flags model msg
application =
    Guida.Kernel.Browser.application
