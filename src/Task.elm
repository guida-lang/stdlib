...
[`andThen`](#andThen). You can use it like `map` if you want:
    ...
    -- guida install guida-lang/stdlib
...
succeed : a -> Task x a
succeed =
    Guida.Kernel.Scheduler.succeed
...
andThen =
    Guida.Kernel.Scheduler.andThen
...
onError =
    Guida.Kernel.Scheduler.onError
...
`Task` does not mean it is done. We must command Guida to `perform` the task:
...
changed a task like "make delicious lasagna" into a command like "Hey Guida, make
...
"Hey Guida, attempt to focus on this DOM node and give me a `Msg` about whether
