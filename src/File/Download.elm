module File.Download exposing (string, bytes, url)
...
url : String -> Cmd msg
url href =
    Task.perform never (Guida.Kernel.File.downloadUrl href)
...
string name mime content =
    Task.perform never (Guida.Kernel.File.download name mime content)
...
bytes name mime content =
    Task.perform never <|
        Guida.Kernel.File.download name mime <|
            Guida.Kernel.File.makeBytesSafeForInternetExplorer content
