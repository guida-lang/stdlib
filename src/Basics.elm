...
import Guida.Kernel.Basics
import Guida.Kernel.Utils
...
conversions (as Scala does) it can be even more confusing. Guida has opted for a
design that makes all conversions explicit.

-}
add : number -> number -> number
add =
    Guida.Kernel.Basics.add
...
**Note:** Guida uses structural equality on tuples, records, and user-defined
...
**Note:** Do not use `(==)` with functions, JSON values, or regular expressions.
It does not work. It will crash if possible. With JSON values, decode to Guida
values before doing any equality checks!

Why is it like this? Equality in the Guida sense can be difficult or impossible
...
remainderBy =
    Guida.Kernel.Basics.remainderBy
...
logBase base number =
    fdiv
        (Guida.Kernel.Basics.log number)
        (Guida.Kernel.Basics.log base)
...
{-| Convert radians to standard Guida angles (radians).
...
{-| Convert degrees to standard Guida angles (radians).
...
{-| Convert turns to standard Guida angles (radians). One turn is equal to 360°.
...
isInfinite =
    Guida.Kernel.Basics.isInfinite
