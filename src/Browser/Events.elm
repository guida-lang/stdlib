...
onVisibilityChange func =
    let
        info =
            Guida.Kernel.Browser.visibilityInfo ()
    in
...
onSelfMsg router { key, event } state =
    let
        toMessage ( subKey, MySub node name decoder ) =
            if subKey == key then
                Guida.Kernel.Browser.decodeEvent decoder event
...
spawn router key (MySub node name _) =
    let
        actualNode =
            case node of
                Document ->
                    Guida.Kernel.Browser.doc

                Window ->
                    Guida.Kernel.Browser.window
    in
    Task.map (\value -> ( key, value )) <|
        Guida.Kernel.Browser.on actualNode name <|
