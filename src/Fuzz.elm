                        ...
                        (\p ->
                            let
                                ...
                                go countdown acc =
                                    case acc of
                                        [] ->
                                            invalid <| "guida-test bug: " ++ functionName ++ " encountered empty list after checking for it."
                    ...
                    |> andThen
                        (\i ->
                            fuzzers
                                ...
                                |> Maybe.withDefault (invalid "guida-test bug: intFrequency index out of range")
                        )
                ...
                |> andThen
                    (\i ->
                        case List.getAt i fuzzers of
                            Nothing ->
                                -- shouldn't happen
                                invalid <| "guida-test bug: " ++ functionName ++ " didn't find a " ++ itemName ++ " at position " ++ String.fromInt i ++ " in the list of length " ++ String.fromInt length ++ "."
        ...
        \prng ->
            case prng of
                Random r ->
                    ...
                    if diceRoll < 0 then
                        Rejected
                            { reason = "guida-test bug: generated a choice < 0"
                            , prng = prng
                            }

                    else if diceRoll > maxValue then
                        Rejected
                            { reason = "guida-test bug: generated a choice > maxChoice"
                            , prng = prng
                            }
                ...
                Hardcoded h ->
                    case RandomRun.nextChoice h.unusedPart of
                        Nothing ->
                            ...
                            Rejected
                                { reason = "guida-test internals: hardcoded PRNG run out of numbers"
                                , prng = prng
                                }
                        ...
                        Just ( hardcodedChoice, restOfChoices ) ->
                            if hardcodedChoice < 0 then
                                ...
                                Rejected
                                    { reason = "guida-test internals: generated a choice < 0"
                                    , prng = prng
                                    }
                                ...
                                Rejected
                                    { reason = "guida-test internals: generated a choice > maxChoice"
                                    , prng = prng
                                    }
...
forcedChoice n =
    Fuzzer <|
        \prng ->
            if n < 0 then
                Rejected
                    { reason = "guida-test bug: forcedChoice: n < 0"
                    , prng = prng
                    }
                        ...
                        case RandomRun.nextChoice h.unusedPart of
                            ...
                            Just ( hardcodedChoice, restOfChoices ) ->
                                if hardcodedChoice /= n then
                                    Rejected
                                        { reason = "guida-test internals: hardcoded value was not the same as the forced one"
                                        , prng = prng
                                        }
