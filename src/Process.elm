...
it needs to, so the long-term vision for concurrency in Guida will be rolling out
...
the Guida community will be supportive of being more ambitious, even if it takes
longer. That’s kind of what Guida is all about.

-}

import Basics exposing (Float, Never)
import Guida.Kernel.Process
...
get a bunch of different tasks running in different processes. The Guida runtime
...
single OS-level thread, Guida can still run things concurrently.
