...
jsToElm =
    Guida.Kernel.Json.unwrap
        >> Guida.Kernel.Debugger.unsafeCoerce
    ...
    let
        className =
            if currentIndex == index then
                "guida-debugger-entry guida-debugger-entry-selected"

            else
                "guida-debugger-entry"

        messageName =
            Guida.Kernel.Debugger.messageToString msg
    in
    div
        ...
        [ span
            [ title messageName
            , class "guida-debugger-entry-content"
            ]
            [ text messageName
            ]
        , span
            [ class "guida-debugger-entry-index"
            ]
...
.guida-debugger-entry:hover {
  background-color: rgb(41, 41, 41);
}

.guida-debugger-entry-selected, .guida-debugger-entry-selected:hover {
  background-color: rgb(10, 10, 10);
}

.guida-debugger-entry-content {
  width: calc(100% - 40px);
  padding: 0 5px;
  box-sizing: border-box;
