...
init : a -> Expando
init value =
    initHelp True (Guida.Kernel.Debugger.init value)
...
merge value expando =
    mergeHelp expando (Guida.Kernel.Debugger.init value)
    ...
    let
        ...
        description =
            case ( maybeName, tinyArgs ) of
                ...
                ( Nothing, x :: xs ) ->
                    text "( " :: span [] x :: List.foldr (\args rest -> text ", " :: span [] args :: rest) [ text " )" ] xs

                ( Just name, [] ) ->
                    [ text name ]

                ( Just name, x :: xs ) ->
                    text (name ++ " ") :: span [] x :: List.foldr (\args rest -> text " " :: span [] args :: rest) [] xs
