...
wrapUpdate update msg model =
    case msg of
        ...
        Open ->
            ( model
            , Task.perform (always NoOp) (Guida.Kernel.Debugger.open model.popout)
            )
...
scroll popout =
    Task.perform (always NoOp) (Guida.Kernel.Debugger.scroll popout)


scrollTo : String -> Popout -> Cmd (Msg msg)
scrollTo id popout =
    Task.perform (always NoOp) (Guida.Kernel.Debugger.scrollTo id popout)


upload : Popout -> Cmd (Msg msg)
upload popout =
    Task.perform Upload (Guida.Kernel.Debugger.upload popout)
...
download metadata history =
    ...
    in
    Task.perform (\_ -> NoOp) <|
        Guida.Kernel.Debugger.download historyLength <|
            Guida.Kernel.Json.unwrap <|
                Encode.object
...
cornerView model =
    Overlay.view
        ...
        (Guida.Kernel.Debugger.isOpen model.popout)
...
viewPlayButton playing =
    button
        ...
        [ if playing then
            icon "M2 2h4v12h-4v-12z M10 2h4v12h-4v-12z"

          else
            icon "M2 2l12 7l-12 7z"
        ]
