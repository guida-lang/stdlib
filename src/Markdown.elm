...
{-| A library for markdown parsing. This is just a Guida API built on top of the
...
{-| Some parser options so you can tweak things for your particular case.
  ...
  - `defaultHighlighting` &mdash; a default language to use for code blocks that do
    not have a language tag. So setting this to `Just "guida"` will treat all
    unlabeled code blocks as Guida code. (This relies on [highlight.js][highlight]
