...
concat tests =
    if List.isEmpty tests then
        ...
        case Internal.duplicatedName tests of
            ...
            Ok _ ->
                Internal.GuidaTestVariant__Batch tests
    ...
    if String.isEmpty desc then
        ...
        case Internal.duplicatedName tests of
            Err dups ->
                ...
                in
                Internal.GuidaTestVariant__Labeled desc <|
                    Internal.failNow
                        { description = String.join "\n" (List.map dupDescription <| Set.toList dups)
                        , reason = Invalid DuplicatedName
                        }
            ...
            Ok childrenNames ->
                if Set.member desc childrenNames then
                    ...
                    Internal.GuidaTestVariant__Labeled desc (Internal.GuidaTestVariant__Batch tests)
...
test untrimmedDesc thunk =
    ...
    if String.isEmpty desc then
        ...
        Internal.GuidaTestVariant__Labeled desc (Internal.GuidaTestVariant__UnitTest (\() -> [ thunk () ]))
...
only : Test -> Test
only =
    Internal.GuidaTestVariant__Only
...
skip : Test -> Test
skip =
    Internal.GuidaTestVariant__Skipped
...
fuzzWithHelp options aTest =
    case aTest of
        Internal.GuidaTestVariant__UnitTest _ ->
            aTest

        Internal.GuidaTestVariant__FuzzTest run ->
            Internal.GuidaTestVariant__FuzzTest (\seed _ -> run seed options.runs)

        Internal.GuidaTestVariant__Labeled label subTest ->
            Internal.GuidaTestVariant__Labeled label (fuzzWithHelp options subTest)

        Internal.GuidaTestVariant__Skipped subTest ->
        ...
        Internal.GuidaTestVariant__Only subTest ->
            fuzzWithHelp options subTest
                |> Internal.GuidaTestVariant__Only
        ...
        Internal.GuidaTestVariant__Batch tests ->
            tests
                ...
                |> Internal.GuidaTestVariant__Batch


{-| Take a function that produces a test, and calls it several (usually 100) times, using a randomly-generated input
from a [`Fuzzer`](http://package.guida-lang.org/packages/guida-lang/stdlib/latest/Fuzz) each time. This allows you to
